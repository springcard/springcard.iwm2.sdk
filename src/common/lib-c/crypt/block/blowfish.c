...
static unsigned long F(BLOWFISH_CTX_ST *ctx, unsigned long x);
...
unsigned long F(BLOWFISH_CTX_ST *ctx, unsigned long x)
{
...
void Blowfish_Encrypt_Ex(BLOWFISH_CTX_ST *ctx, unsigned long *xl, unsigned long *xr)
{
  unsigned long  Xl;
  unsigned long  Xr;
  unsigned long  temp;
...
void Blowfish_Decrypt_Ex(BLOWFISH_CTX_ST *ctx, unsigned long *xl, unsigned long *xr)
{
  unsigned long  Xl;
  unsigned long  Xr;
  unsigned long  temp;
...
void Blowfish_Init(BLOWFISH_CTX_ST *ctx, const unsigned char *key, int keyLen)
{
  int i, j, k;
  ...
  for (i = 0; i < N + 2; i += 2)
  {
    Blowfish_Encrypt_Ex(ctx, &datal, &datar);
    ctx->P[i] = datal;
    ctx->P[i + 1] = datar;
  }
...
{
  Blowfish_Encrypt_Ex(blowfish_ctx, (unsigned long *) &data[0], (unsigned long *) &data[4]);
}

void Blowfish_Decrypt(BLOWFISH_CTX_ST *blowfish_ctx, BYTE data[8])
{
  Blowfish_Decrypt_Ex(blowfish_ctx, (unsigned long *) &data[0], (unsigned long *) &data[4]);
}
...
/* Blowfish test vectors - http://www.schneier.com/code/vectors2.txt */
...
unsigned char set_key[NUM_SET_KEY_TESTS] = {
...
BOOL Blowfish_SelfTest(void)
{
  ...
  Blowfish_Encrypt_Ex(&ctx, &L, &R);
  if (L != 0xDF333FD2L || R != 0x30A71BB4L)
  {
	  printf("Blowfish first test failed : %08lX %08lX\n", L, R);
    return FALSE;
  }

  Blowfish_Decrypt_Ex(&ctx, &L, &R);
  if (L != 1 || R != 2)
  {
	  printf("Blowfish second test failed : %08lX %08lX\n", L, R);
    return FALSE;
  }
	...
	for (i=0; i<NUM_VARIABLE_KEY_TESTS; i++)
  {
    ...
    {
	    printf("Blowfish cipher %d failed : %08lX %08lX != %08lX %08lX\n", i, L, R, ciphertext_l[i], ciphertext_r[i]);
      return FALSE;
    }
    ...
    {
	    printf("Blowfish decipher %d failed : %08lX %08lX != %08lX %08lX\n", i, L, R, plaintext_l[i], plaintext_r[i]);
      return FALSE;
    }
	}
  ...
  {
    /* Load the key */
    Blowfish_Init(&ctx, set_key, 1+i-NUM_VARIABLE_KEY_TESTS);
