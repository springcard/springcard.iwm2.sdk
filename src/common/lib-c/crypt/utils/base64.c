...
void
buffer_add (struct buffer_st *b, char c)
{
  *(b->ptr++) = c;
  b->offset++;
  if (b->offset == (int) b->length)
    {
      b->length += 512;
...
void
buffer_delete (struct buffer_st *b)
{
  free (b->data);
  b->length = 0;
  b->offset = 0;
...
void
base64_encode (struct buffer_st *b, const char *source, size_t length)
{
  int i, hiteof = 0;
  int offset = 0;
  int olen;
...
void
base64_decode (struct buffer_st *bfr, const char *source, size_t length)
{
  int i;
  int offset = 0;
  int endoffile;
