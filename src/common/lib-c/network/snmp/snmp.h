...
#ifdef HAVE_IPX
# define SNMP_PORT_IPX       36879
# define SNMP_TRAP_PORT_IPX  36880
#endif

#define SNMP_MAX_LEN	    1450
#ifdef HAVE_IPX
 #define SNMP_MAX_LEN_IPX    546
#endif
...
extern void init_snmp _ANSI_ARGS_((void));

extern int read_objid _ANSI_ARGS_((char *input,
				   oid *output,
				   int	*out_len));

extern void print_objid _ANSI_ARGS_((oid *id,
				     int idlen));

extern void snmp_add_null_var _ANSI_ARGS_((struct snmp_pdu *pdu,
					   oid *name,
					   int name_length));

extern void xdump _ANSI_ARGS_((u_char *cp,
			       int length,
			       char *prefix));

extern void snmp_synch_setup _ANSI_ARGS_((struct snmp_session *session));

extern int snmp_synch_response _ANSI_ARGS_((struct snmp_session *ss,
					    struct snmp_pdu *pdu,
