 ...
 */
static struct variable_list *clone_variable(struct variable_list *var)
{
  ...
  if (var->val.string != NULL){
    newvar->val.string = (u_char *)calloc(1, var->val_len + 1);
...
int snmp_synch_input(int op, struct snmp_session *session, int reqid, struct snmp_pdu *pdu, void *magic)
{
...
{
  session->callback = (void *) &snmp_synch_input;
