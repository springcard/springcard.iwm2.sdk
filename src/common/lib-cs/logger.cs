...
namespace SpringCard.LibCs
{
	public class Logger
	{
		public enum Level { None = -1, Fatal, Error, Warning, Info, Trace, Debug, All };
    ...
    private static object locker = new object();
		...
		public static void LoadConfigFromRegistry(string CompanyName, string ProductName)
		{
			try
			{
				...
				{
					if (i >=  (int) Level.Debug) verboseLevel = Level.Debug; else
					if (i >=  (int) Level.Trace) verboseLevel = Level.Trace; else
					if (i >=  (int) Level.Info) verboseLevel = Level.Info; else
					if (i >=  (int) Level.Warning) verboseLevel = Level.Warning; else
					if (i >=  (int) Level.Error) verboseLevel = Level.Error; else
					if (i >=  (int) Level.Fatal) verboseLevel = Level.Fatal;
				}
				else
				{
